...
class WalkerBot():
    ...
    def __init__(self, bot: Bot, morg_api: MorgHTTPSocket = MorgHTTPSocket()) -> None:
        self.bot = bot
        self.morg_api = morg_api

    def wait_for_idle(self):
        while not self.morg_api.get_is_player_idle():
            time.sleep(1)
    ...
    def walk(self, path: Path, destination: Area) -> None:
        ...
        """
        self.bot.log_msg("Started walking")
        ...
        while True:
            ...
            if self.is_at_destination(destination):
                self.bot.log_msg("We made it!")
                break

            self.bot.log_msg("not at dest")
    ...
    def walk_to_area(self, destination: Area):
        ...
        while True:
            ...
            self.bot.log_msg(f"Moving to new position {new_pos}")
    ...
    def get_next_pos(self, destination: Area):
        current_pos = self.morg_api.get_player_position()
    ...
    def change_position(self, new_pos: List[int]) -> None:
        ...
        center_minimap = self.bot.win.minimap.get_center()
    ...
    def get_target_pos(self, path: Path) -> List[int]:
        ...
        """
        current_position = self.morg_api.get_player_position()
    ...
    def compute_tiles(self, new_x: int, new_y: int) -> List[float]:
        ...
        camera_position = self.morg_api.get_camera_position()
