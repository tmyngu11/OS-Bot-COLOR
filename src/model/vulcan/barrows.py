...
class VulcanBarrows(VulcanBot):
    ...
    def __handle_combat(self):
        time.sleep(5)
        if self.get_is_player_idle() and self.get_is_in_combat():
    ...
    def __swap_equip(self, brother):
        required_equip = item_ids.IBANS_STAFF_U
        if brother == "Ahrim":
            required_equip = item_ids.RUNE_CROSSBOW
        if not self.get_is_item_equipped(required_equip):
            item = self.get_first_occurrence(required_equip)
    ...
    def __eat_food(self):
        while self.get_hp() < 60:
            food_indexes = self.get_inv_item_indices(item_ids.all_food)
            if food_indexes:
                self.log_msg("Eating...")
